...
pub struct Editor {
    ...
    pub mode: Mode,
    pub tree: Tree,
    pub split_info: HashMap<String, persistence::SplitEntryTree>,
...
impl Editor {
    ...
    fn get_split_tree(
        &self,
        focus_id: ViewId,
        tree_info: TreeInfoTree,
    ) -> persistence::SplitEntryTree {
        match tree_info {
            TreeInfoTree::Leaf(view_id) => {
                persistence::SplitEntryTree::Leaf(self.get_path_from_view_id(view_id).map(|path| {
                    let doc = self.tree.try_get(view_id).unwrap().doc;
            ...
            TreeInfoTree::Node(node) => {
                ...
                let layout = match node.layout {
                    tree::Layout::Horizontal => persistence::Layout::Horizontal,
                    tree::Layout::Vertical => persistence::Layout::Vertical,
                };

                persistence::SplitEntryTree::Node(persistence::SplitEntryNode { layout, children })
            }
        }
    }
    ...
    fn load_split_tree(
        &mut self,
        focus: ViewId,
        split_tree: &persistence::SplitEntryTree,
    ) -> Option<ViewId> {
        self.focus(focus);

        match split_tree {
            persistence::SplitEntryTree::Leaf(leaf) => {
                let leaf = match leaf {
                    Some(l) => l,
                    None => return None,
                };
            ...
            persistence::SplitEntryTree::Node(node) => {
                let mut view_child_pairs = Vec::with_capacity(node.children.len());
                for child in &node.children {
                    let layout = match node.layout {
                        persistence::Layout::Horizontal => Action::HorizontalSplit,
                        persistence::Layout::Vertical => Action::VerticalSplit,
                    };
    ...
    pub fn load_split(&mut self, split_name: String) -> anyhow::Result<()> {
        ...
        let split_entry_tree = match self.split_info.get(&split_name) {
            Some(se) => se,
            None => anyhow::bail!(format!("Split '{}' doesn't exist.", split_name)),
        };
